...
class RegisterView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if registration_form.is_valid():
            ...
            login(request, user)
            messages.success(request, f' User Registered Successfully ! {user.username}')
            return redirect('home')
        else:
            messages.error(request, ' Unable to register ! ')
            return render(request, 'views/register.html', {'register_form': registration_form})
...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if user_form.is_valid() and profile_form.is_valid() and location_form.is_valid():
            ...
            messages.error(request, ' Unable to register the profile ! ')
        return render(request, 'views/profile.html',
                      {'user_form': user_form, 'profile_form': profile_form, "location_form": location_form,
                       'user_listings': user_listings, 'user_liked_listings': user_liked_listings, })
